...
const baseUrl = import.meta.env.VITE_BACKEND_URL + "/api/tournament/";
  ...
  try {
    ...
    if (error.response && error.response.status === 400) {
      ...
      error.response.data.message ===
        "Player is not allowed to create tournament" &&
...
const joinTournament = async (tournamentId) => {
  try {
    const response = await api.post("/addPlayer", {
    ...
    if (
      ...
      error.response.data.message === "Player already added to tournament" &&
      error.response.status === 400
    ) {
...
const leaveTournament = async (tournamentId) => {
  try {
    const response = await api.post("/removePlayer", {
      tournamentId: tournamentId,
    });
    if (response.status === 200) {
      return response.data;
    ...
    if (
      ...
      error.response.data.message === "Player not in tournament" &&
      error.response.status === 400
    ) {
...
export const deleteTournament = async (tournamentId) => {
  try {
    const response = await api.get("/delete/" + tournamentId);
    if (response.status === 200) {
      return response.data;
    }
  } catch (err) {
