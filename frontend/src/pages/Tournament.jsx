...
import Ongoing from "../componenets/tournamentDisplays/Ongoing";
...
export default function Tournament() {
  ...
  useEffect(() => {
    const fetchTournament = async () => {
      try {
        ...
        setIsJoined(
          data.tournament.playerIds.find((p) => p._id == playerData._id)
        );

        setCanEdit(
          playerData.accountType === "admin" ||
            data.tournament.organizerId === playerData._id
        );
      } catch (error) {
  ...
  const handleStart = async (e) => {
    e.preventDefault();
    const started = await startTournament(id);
    if (started) {
      navigate("/app/tournament/" + id);
    }
  };
    ...
    <div className="bg-[#011534] flex flex-row min-w-screen min-h-screen">
      ...
      <div className="text-white p-4 flex flex-col min-w-[calc(85%)] min-h-screen text-center">
        ...
        {tournament.status == "ongoing" && (
          <Ongoing tournament={tournament} canEdit={canEdit} />
        )}
      </div>
    </div>
  );
