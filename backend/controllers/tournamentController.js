...
export const addPlayerToTournament = async (req, res) => {
  ...
  try {
    ...
    if (tournament.playerIds.includes(playerId)) {
      return res
        .status(400)
        .json({ message: "Player already added to tournament" });
    }
    ...
    res
      .status(200)
      .json({ message: "Player added to tournament successfully", tournament });
  } catch (error) {
...
export const tournamentNextMatch = async (
  tournamentId,
  lastMatchId,
  currentRound
) => {
  try {
    ...
    await setMatchDate(
      nextMatch._id,
      new Date(Date.now() + tournament.timeBetweenMatch * 1000)
    );
    return nextMatch;
  } catch (error) {
...
export const tournamentNextRound = async (tournamentId, currentRound) => {
  try {
    ...
    const nextRoundMatches = await Promise.all(
